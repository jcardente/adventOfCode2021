...
# of course it is), and it even has an experimental antenna that
# should be able to track the keys if you can boost its signal
# strength high enough; there's a little meter that indicates the
...
# marked B (200, 208, 210); its sum is 618. The sum of measurements in
...
# sum. So, compare A with B, then compare B with C, then C with D, and
# so on. Stop when there aren't enough measurements left to create a
